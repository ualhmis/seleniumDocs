...
ls -la  $JENKINS_HOME/selenium-drivers/ <4>
sudo chown jenkins:jenkins $JENKINS_HOME/selenium-drivers/ <5>
...
<4> Lista los archivos en la carpeta `selenium-drivers`. Comprueba que los permisos no son adecuados para que Jenkins pueda lanzar los ejecutables de los drivers. 
<5> Asigna el grupo `jenkins:jenkins` a la carpeta y los archivos. Como resultado los archivos deben tener los permisos correctos
...
Realiza la instalación de Firefox descargando la última versión desde la página oficial de Mozilla.

[WARNING]
====
No utilices el gestor de paquetes `apt` para instalar Firefox, ya que en algunas versiones de Ubuntu, la versión de Firefox que se instala es la versión de Snap, y esta no es compatible con Jenkins. 
====

[source%nowrap,bash]
----
cd /opt
sudo wget -O firefox.tar.xz "https://download.mozilla.org/?product=firefox-latest&os=linux64&lang=en-US"
sudo tar -xf firefox.tar.xz
sudo ln -s /opt/firefox/firefox /usr/local/bin/firefox
----

Comprueba la instalacion de firefox: 

[source%nowrap,bash]
----
firefox --version
sudo -u jenkins firefox --version
----

Bloquea Snap para Firefox:
[source%nowrap,bash]
----
sudo apt-mark hold firefox
----

Después de instalar, haz esta prueba:
[source%nowrap,bash]
----
sudo -u jenkins firefox --headless --version
----

Debería funcionar sin errores, y mostrar la versión de Firefox instalada. Si no es así, revisa los pasos anteriores.
    ...
    stages {
        stage('Git clone') {
            steps{
                ...
                git branch: 'main', url: 'https://github.com/tu-usuario/tu-repo-selenium.git'
            }
        }
        stage('Firefox tests') {
            steps {
...
Para probar la ejecución con Chrome, debes instalar Chrome en la máquina Jenkins.

. Descargar el paquete oficial de Google Chrome para Linux:
...
[start=3]
. Verificar instalación, también desde el usuario `jenkins`:
[source%nowrap,bash]
----
google-chrome --version
sudo -u jenkins google-chrome --version
----

Comprueba la versión del Chrome instalado, porque a continuación tienes que descargar el `chromedriver` adecuado para esa versión de Chrome. 


[start=4]
. Descargar el chrome driver para linux, y configurar la ejecución de los tests con Chrome en modo _headless_

[source%nowrap,bash,linenums]
----
wget https://storage.googleapis.com/chrome-for-testing-public/147.0.7727.137/linux64/chromedriver-linux64.zip <1>
sudo apt-get install unzip -y 
sudo unzip chromedriver-linux64.zip 
sudo cp chromedriver-linux64/chromedriver $JENKINS_HOME/selenium-drivers/ <2>
sudo chown jenkins:jenkins $JENKINS_HOME/selenium-drivers/ 
sudo chown jenkins:jenkins $JENKINS_HOME/selenium-drivers/*
----
<1> Con `wget` descarga `chromedriver` para linux en la carpeta home. En este caso se ha descargado la versión 147.0.7727.137, porque es la version de Chrome instalada en el paso anterior. *IMPORTANTE*: revisa la https://www.ubuntuupdates.org/package/google_chrome/stable/main/base/google-chrome-stable[última versión estable] de Chrome para Ubuntu y usa la https://googlechromelabs.github.io/chrome-for-testing/[URL de descarga] de `chromedriver_linux64.zip` adecuada para la versión estable de Chrome
<2> Con `unzip` descomprime en la carpeta deseada. Además has tenido que instalar `unzip`.

[start=5]
. Después, crea una nueva fase donde llames a los tests con el driver de Chrome. Tendrás que modificar el driver en el código, y llamar a maven con el siguiente parámetro para Chrome Driver: 
...
=== Pipeline con navegadores en paralelo
