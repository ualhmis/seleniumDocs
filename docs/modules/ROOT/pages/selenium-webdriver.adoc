...
. En Eclipse, crea un nuevo proyecto *Maven*. Selecciona la opción _Create a simple project (skip archetype selection)_

.Nuevo proyecto Maven
image::eclipse-new-maven-project.png[role="thumb", align="center", width="50%"]

[start=2]
. Introduce los datos para el proyecto Maven: 
...
image::eclipse-new-maven-project-artifactid.png[role="thumb", align="center", width="50%"]

[start=3]
. En la carpeta `test` crea un nuevo paquete en la carpeta test de nombre `org.ual.hmis.nombreEquipo` (sustituyendo `nombreEquipo` por el nombre de tu equipo). Ahí guarda los archivos `.java` exportados de Selenium IDE. 


. Verás que aparecen muchos errores de compilación porque aun no hemos añadido las librerías necesarias de JUnit 4 y Selenium Webdriver. Para hacerlo, vamos a apoyarnos en Maven. 
...
.Editar el `pom.xml`
image::eclipse-edit-pom.png[role="thumb", align="center", width="50%"]
...
----
<1> JUnit 4. 
<2> Selenium-Java, librería básica de Selenium Webdriver. Puedes ver la última versión disponible aquí: https://mvnrepository.com/artifact/org.seleniumhq.selenium/selenium-java[Selenium Java on Maven Repository,window=_blank]
<3> Selenium Firefox Driver, librería con el driver de Firefox para poder ejecutar los tests en este navegador. Puedes ver la última versión disponible aquí: https://mvnrepository.com/artifact/org.seleniumhq.selenium/selenium-firefox-driver[Selenium Firefox Driver on Maven Repository,window=_blank]
<4> Selenium Chrome Driver, librería con el driver de Chrome para poder ejecutar los tests en este navegador. Puedes ver la última versión disponible aquí: https://mvnrepository.com/artifact/org.seleniumhq.selenium/selenium-chrome-driver[Selenium Chrome Driver on Maven Repository,window=_blank]
...
====
Si estas en Windows, es importante que descargues la versión `geckodriver-vX.XX.X-win64.zip` y no la versión `geckodriver-vX.XX.X-win32.zip`, ya que esta última da problemas de ejecución.

.Primero muestra todos los archivos
image::geckodriver-64-donwload-1.png[role="thumb", align="center"]

.Después selecciona la versión adecuada para tu sistema
image::geckodriver-64-donwload-2.png[role="thumb", align="center"]
====
...
----
  ...
  public void setUp() {
    ...
    case 0:  // firefox
      ...
      // Asegurate de descargar la versión adecuada para tu sistema, por ejemplo `geckodriver-vX.XX.X-win64.zip` para Windows.
    ...
    case 1: // chrome
    	...
    	if (headless) chromeOptions.addArguments("--headless=new"); // .setHeadless(headless);
...
Selenium tiene varias https://www.selenium.dev/documentation/webdriver/waits/[estrategias de espera,window=_blank], principalmente https://www.selenium.dev/documentation/webdriver/waits/#explicit-wait[esperas explícitas,window=_blank] y https://www.selenium.dev/documentation/webdriver/waits/#implicit-wait[esperas implícitas,window=_blank]. Podemos añadir esperas explícitas de dos formas para que el código detenga la ejecución del programa, o congelar el hilo, hasta que la condición que le pases se resuelva: 
...
Otro método de espera es la espera implícita, que aparece en la documentación de Selenium y en los ejemplos. Por ejemplo el método https://www.selenium.dev/selenium/docs/api/java/org/openqa/selenium/WebDriver.Timeouts.html#implicitlyWait-long-java.util.concurrent.TimeUnit-[implicitlyWait]: 

    driver.manage().timeouts().implicitlyWait(Duration.ofSeconds(10));

Establece la cantidad de tiempo que el driver debería esperar cuando busca un elemento si este no está presente inmediatamente. Sin embargo, no funciona siempre como se espera si no se están usando las últimas versiones de las dependencias y de los navegadores, y los errores no se solucionan.
...
https://github.com/SeleniumHQ/htmlunit-driver[HtmlUnit Driver] es un driver _headless_ nativo para Selenium Webdriver. Se trata de una implementación en Java de Webdriver. 

Para usarlo, hay que añadir la https://mvnrepository.com/artifact/org.seleniumhq.selenium/htmlunit-driver[dependencia a HtmlUnit Driver] en el pom: 
...
- Actualización de Selenium 3 a https://www.selenium.dev/documentation/webdriver/getting_started/upgrade_to_selenium_4[Selenium 4]
